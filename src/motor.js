...
export class Motor extends EventEmitter   {
  ...
  constructor({ id, stepper, board, stepPin, dirPin, limitPin, encoderPinA, encoderPinB, limPos, limNeg, stepDeg, enablePin, limitDir = FORWARDS, invertEnable = true, limitAdj = 0, maxSpeed, maxAccel }) {
    ...
    this.maxSpeed = maxSpeed ?? 2200;           // the max speed for this motor in steps/s
  ...
  setPosition( position, speed = this.maxSpeed, acceleration = this.maxAccel){
    ...
    logger(`motor ${this.id} set position to ${position}º speed ${speed} steps/s with accel ${acceleration}`);
